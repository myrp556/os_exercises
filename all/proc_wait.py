...
class scheduler:
	...
	return
    def move_to_waitting(self, expected, pid=-1):
        #YOUR CODE
	if pid == -1:
            pid = self.curr_proc;
	proc = self.proc_info[pid];
	if proc[PROC_STATE] == expected:
            proc[PROC_STATE] = STATE_WAIT;
	return		
    ...
    def run(self):
        ...
        while self.get_num_active() > 0:
            ...
            if self.proc_info[self.curr_proc][PROC_STATE] == STATE_RUNNING and \
                ...
                #YOUR CODE
                instruction_to_execute = self.proc_info[self.curr_proc][PROC_CODE].pop(0);#remove instruction to be excuted.
		if len(self.proc_info[self.curr_proc][PROC_CODE])==0:
                    self.move_to_done(STATE_RUNNING);
		...
		self.move_to_waitting(STATE_RUNNING);
		self.next_proc();
