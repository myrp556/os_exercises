...
>  IDT初始化之后需要设置时钟中断和中断描述符。完成后需要初始化8259中断控制器之后初始化8253时钟外设。
...
- [x]  
- 
>使用两个数组。一个数组用作系统调用，另外一个数组用来存储相应的调用次数。每次一个系统调用的时候都会选择到相应的另外一个数组的记录值，每次调用后就把这个数值+1。如此可以记录并且获取调用的次数信息。
